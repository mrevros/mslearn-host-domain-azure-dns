...
az network nsg rule create -g $RgName --nsg-name MTPortalNSG -n AllowAll80 --priority 101 \
...
for i in `seq 1 2`; do
  ...
  az network nic create \
    --resource-group $RgName \
    --name webNic$i \
    --vnet-name MTPortalVnet \
    --subnet MTPortalSubnet \
    --network-security-group MTPortalNSG \
    --location $Location
done 
